...
import { sendSMSVerificationCode, verifySMSCode, sendEmailVerificationLink, getVerificationStatus, verifyNin } from '@/api/verification'
import { updateProfile } from '@/api/profile'
import { CheckCircle, Clock, AlertCircle, Phone, Mail, Shield, CreditCard } from 'lucide-react'
...
export function KYCVerification() {
  ...
  const [ninVerificationResult, setNinVerificationResult] = useState<any>(null)
  ...
  const fetchVerificationStatus = async () => {
    try {
      ...
      if (response.data && response.data.success) {
        ...
        // Set NIN verification result if available
        if (status.ninData) {
          setNinVerificationResult(status.ninData)
        }
        ...
        } else if (!status.isNinVerified) {
          setActiveStep(3) // NIN verification
        } else {
  ...
  const handleVerifyNin = async (e: React.FormEvent) => {
    ...
    if (!nin.trim() || nin.length !== 11 || !/^\d{11}$/.test(nin)) {
      toast({
        title: "Error",
    ...
    try {
      setNinLoading(true)
      console.log('KYCVerification: Verifying NIN')

      const additionalData: any = {}
      if (fullname.trim()) additionalData.fullname = fullname.trim()
      if (dateOfBirth.trim()) additionalData.dateOfBirth = dateOfBirth.trim()

      const response = await verifyNin(nin, additionalData)
      console.log('KYCVerification: NIN verification response:', response)
      ...
      if (response.data && response.data.success) {
        ...
        setNinVerificationResult(response.data.data.verificationData)
      ...
      console.error('KYCVerification: Error verifying NIN:', error)
      toast({
        title: "Error",
        description: error.message || "Failed to verify NIN",
        variant: "destructive",
      })
    } finally {
      setNinLoading(false)
        ...
        <Card className="max-w-2xl mx-auto">
          ...
          <CardContent className="text-center space-y-4">
            <div className="flex justify-center space-x-4 flex-wrap gap-2">
              ...
              {getVerificationBadge(verificationStatus.isNinVerified, 'NIN')}
            </div>

            {ninVerificationResult && (
              <Card className="mt-4">
                <CardHeader>
                  <CardTitle className="text-lg">Verified Identity Information</CardTitle>
                </CardHeader>
                <CardContent className="space-y-2 text-left">
                  <div><strong>Full Name:</strong> {ninVerificationResult.fullname}</div>
                  <div><strong>Date of Birth:</strong> {ninVerificationResult.dateOfBirth}</div>
                  <div><strong>Gender:</strong> {ninVerificationResult.gender}</div>
                  <div><strong>State of Origin:</strong> {ninVerificationResult.stateOfOrigin}</div>
                  <div><strong>Local Government:</strong> {ninVerificationResult.localGovernment}</div>
                </CardContent>
              </Card>
            )}
              ...
              <Step>
                <StepLabel>
                  <div className="flex items-center space-x-2">
                    ...
                    <span>Verify National ID (NIN)</span>
                    {verificationStatus?.isNinVerified && <CheckCircle className="h-4 w-4 text-green-600" />}
                  </div>
                </StepLabel>
                <StepContent>
                  <form onSubmit={handleVerifyNin} className="space-y-4">
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                      <div className="space-y-2">
                        ...
                        <Input
                          ...
                          placeholder="Enter your full name for verification"
                        />
                      </div>
                      <div className="space-y-2">
                      ...
                      <div className="flex items-start space-x-2">
                        ...
                        <div>
                          ...
                          <p className="text-sm text-yellow-600">
                            Your NIN will be securely verified using the official NinVerify service.
                    ...
                    <Button type="submit" disabled={ninLoading || nin.length !== 11}>
                  ...
                  {ninVerificationResult && (
                    <Card className="mt-4">
                      <CardHeader>
                        <CardTitle className="text-lg text-green-600">✓ NIN Verified Successfully</CardTitle>
                      </CardHeader>
